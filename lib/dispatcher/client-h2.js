...
const kRequestBodyWriterDone = Symbol('request body writer done')
...
// Cap Undici's automatic GOAWAY recovery at one replay. Without this policy,
// a peer that refuses every connection can turn one request into an unbounded
// connect/refuse/reconnect loop that never settles and starves the event loop.
...
function canReplayRequest (request) {
  ...
  return body == null || util.isBuffer(body) || util.isBlobLike(body) || body?.[kBodyReplayable] === true
}
...
function cancelBodyWriter (state) {
  const writer = state.bodyWriter
  if (writer == null) {
    return
  }

  state.bodyWriter = null
  state.request[kRequestBodyWriterDone] = writer.done

  try {
    writer.iterator.return?.()?.catch?.(noop)
  } catch {}
}
...
function retryRefusedStream (stream, state) {
  ...
  // RFC 9113 section 8.7 permits retrying REFUSED_STREAM. Undici limits this
  // automatic recovery to one attempt to avoid an unbounded refusal loop.
...
async function writeIterable (abort, h2stream, body, client, request, socket, contentLength, expectsPayload) {
  ...
  const state = h2stream[kRequestStreamState]
  let callback = null
  let writer = null
  ...
  try {
    const previousWriterDone = request[kRequestBodyWriterDone]
    if (previousWriterDone != null) {
      await previousWriterDone
      if (request[kRequestBodyWriterDone] === previousWriterDone) {
        request[kRequestBodyWriterDone] = null
      }
    }

    if (request[kRequestStream] !== h2stream) {
      return
    }
    ...
    writer = { done, exhausted: false, iterator, resolve: resolveWriter }
    state.bodyWriter = writer
