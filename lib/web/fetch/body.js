...
function createBodyStream (action) {
  let cancelled = false
  let controller = null
  let iterator = null
  let iteratorClosed = false

  function closeIterator (reason) {
    if (iterator == null || iteratorClosed) {
      return
    }
  ...
  ;(async () => {
    try {
      ...
      if (iterator) {
        while (true) {
          if (cancelled) break
          const { done, value: bytes } = await iterator.next()
          if (done) {
            iteratorClosed = true
            break
          }
          if (cancelled || isErrored(stream)) break
          if (bytes.length) {
            controller.enqueue(new Uint8Array(bytes))
          }
        }
      } else if (!cancelled && result?.length && !isErrored(stream)) {
 ...
 * @returns {[{stream: ReadableStream, source: any, length: number | null, replay: (() => ReadableStream) | null}, string | null]} - Returns a tuple containing the body and its type
 *
...
function extractBody (object, keepalive = false) {
  ...
  if (stream == null) {
    assert(action != null)
    replay = () => createBodyStream(action)
    stream = replay()
  }
  ...
  const body = { stream, source, length, replay }
 ...
 * @property {(() => ReadableStream) | null} replay - Creates a fresh stream over the originally extracted bytes
 */
