...
const { ReplayableBody } = require('./replayable-body')
const {
  redirectStatusSet,
  nullBodyStatus,
  safeMethodsSet,
...
async function httpNetworkFetch (
  ...
  if (request.body == null && fetchParams.processRequestEndOfBody) {
    ...
    if (request.body.replay != null) {
      // Source-backed bodies retain a factory over the bytes captured during
      // extraction. Present that capability to core so HTTP/2 can replay a
      // request that the server did not process while preserving Fetch body
      // progress callbacks for each transmission attempt.
      requestBody = new ReplayableBody(
        request.body.stream,
        request.body.length,
        request.body.replay,
        {
          isCancelled: () => isCancelled(fetchParams),
          onChunk: (len) => fetchParams.processRequestBodyChunkLength?.(len),
          onEnd: processEndOfBody,
          onError: processBodyError
        }
      )
    } else {
